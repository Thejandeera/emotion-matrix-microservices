...
1. **Frontend (Next.js):** Provides a UI for static audio file uploads and live microphone recording.
2. **API Gateway (Port 8000):** The central orchestrator. Routes traffic and bundles the final JSON payload.
3. **Whisper STT Service (Port 8001):** Handles Speech-to-Text using `faster-whisper-large-v3-turbo-ct2` with Voice Activity Detection (VAD).
4. **Phrase Extraction Service (Port 8002):** A lightweight NLP pipeline using spaCy's `PhraseMatcher` to isolate exact sentence boundaries around target keywords with sub-millisecond latency.
5. **Sentiment Service (Port 8003):** Utilizes a fine-tuned RoBERTa model (`SamLowe/roberta-base-go_emotions`) to categorize the emotion (e.g., Disappointment, Anger, Joy) and overall sentiment of the isolated sentences.
...
Ensure your Python virtual environment (`venv`) is fully set up with all requirements installed.
